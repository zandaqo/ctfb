...
class CTFB {
  ...
  async combine() {
    ...
    ffmpeg.stdout.on('data', (data) => {
      ...
      if (match) this.constructor.printProgress(match[1], totalTime);
    });
    ffmpeg.on('exit', () => {
  ...
  static printProgress(currentTime, totalTime) {
    const progress = ((parseInt(currentTime, 10) / totalTime) * 100).toFixed(3) + '%';
    process.stdout.clearLine();
    process.stdout.cursorTo(0);
    process.stdout.write(`Progress: ${progress}    Time left: `);
  }
