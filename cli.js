...
const options = {
  ...
  'pattern': { question: 'Input Pattern: ', answer: '*.mp3' },
  'output': { question: 'Output File: ', answer: path.basename(process.cwd()) + '.opus' },
  'bitrate': { question: 'Bitrate: ', answer: '64k' },
};
  ...
  for (const option of Object.keys(options)) {
    if (!Reflect.has(answers, option)) {
      if (isInteractive) {
        ...
        answers[option] = option === 'pattern' ? [options.pattern.answer] : options[option].answer;
      }
    }
  }
